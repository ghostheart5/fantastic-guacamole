...
SOURCE_HASH = '4d0280658d5cba6c6b94859a9234ccdb194267b43c2cdfa51a50080dd8cb3617'
...
'''
REPLACEMENTS = [
    ("        Write-Host 'Stopping the matrix at the first failed variant to preserve time and evidence.' -ForegroundColor Yellow\n        break", "        Write-Host 'Variant failed; collecting the remaining variants with all failure gates retained.' -ForegroundColor Yellow"),
...
def prepare(source):
    ...
    receipt = {'originalCanonicalSha256':digest(original),'executedCanonicalSha256':digest(changed),
               ...
               'extensions':['explicit frozen source root for isolated copy','Nexus UI bootstrap before each ready variant',
                             'record authenticated-start result', 'collect remaining variants while retaining matrix failure']}

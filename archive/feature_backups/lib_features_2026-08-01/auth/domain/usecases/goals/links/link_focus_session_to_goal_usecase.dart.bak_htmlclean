...
class LinkFocusSessionToGoalUsecase {
  const LinkFocusSessionToGoalUsecase(
  ...
  Future<TimelineEventEntity?> call({
    ...
    final TimelineEventEntity event = TimelineEventEntity(
      id: 'goal.link.focus_session${targetGoalId}_${targetItemId}_${now.microsecondsSinceEpoch}',
      type: TimelineEventType.task,
      title: 'Focus session linked to goal',
      detail: 'Focus session linked $targetItemId for goal "${goal.title}".',
      timestamp: now,
      status: TimelineEventStatus.info,
      phase: 'goal.link.focus_session',

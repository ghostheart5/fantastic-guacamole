...
class CreateLearningGoalUsecase {
  const CreateLearningGoalUsecase(this._repository);
  ...
  Future<GoalEntity> call({
    ...
    final GoalEntity goal = GoalEntity(
      id: 'learning_goal_${now.microsecondsSinceEpoch}',
      title: title.trim().isEmpty ? 'Learning goal' : title.trim(),
      createdAt: now,
      description: description?.trim().isEmpty ?? true
          ? 'Learning goal created in ChronoSpark.'
          : description?.trim(),

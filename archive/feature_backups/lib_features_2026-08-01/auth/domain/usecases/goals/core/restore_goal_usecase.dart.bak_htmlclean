...
class RestoreGoalUsecase {
  const RestoreGoalUsecase(this._goalRepository, this._timelineRepository);
  ...
  Future<TimelineEventEntity?> call(String goalId) async {
    ...
    final TimelineEventEntity event = TimelineEventEntity(
      id: 'goal_restore_${goal.id}_${now.microsecondsSinceEpoch}',
      type: TimelineEventType.reflection,
      title: 'Goal restored',
      detail: 'Restored goal "${goal.title}".',
      timestamp: now,
      status: TimelineEventStatus.info,
      phase: 'goal.metadata.restore',
      relatedId: goal.id,
    );

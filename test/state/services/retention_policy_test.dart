...
void main() {
  group('RetentionPolicy', () {
    ...
    test('session expiration honors 30-day window', () {
      ...
      final DateTime withinWindow = now.subtract(
        const Duration(days: 29, hours: 23),
      );
      final DateTime outsideWindow = now.subtract(
        const Duration(days: 30, minutes: 1),
      );

      expect(
        RetentionPolicy.standard.isSessionExpired(withinWindow, now: now),
        isFalse,
      );
      expect(
        RetentionPolicy.standard.isSessionExpired(outsideWindow, now: now),
        isTrue,
      );
    });
  });

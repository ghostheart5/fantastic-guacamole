...
void main() {
  ...
  test('nextActionTextProvider uses first ranked task title', () async {
    ...
    expect(container.read(nextActionTextProvider), 'Focus on: Lock release scope');
  });
  ...
  test('AI notifier providers update state deterministically', () {
    ...
    container.read(aiPersonalityProvider.notifier).set(AIPersonality.strategist);
    container.read(aiInputProvider.notifier).set('What is next?');
    container
        .read(aiExecutionStatusProvider.notifier)
        .set(const AIExecutionStatus(phase: 'running', requestId: 'req-1', durationMs: 55));

...
void main() {
  test(
    'real AI producer and prediction/insight readers remain account scoped',
    () async {
      ...
      expect(
        (await _readPrediction(container, 'shared-task')).outcome,
        'Likely Success',
      );
      expect(
        await _readInsight(container),
  ...
  test(
    'real task completion producer writes only the active account history',
    () async {
      final ControllableSecureStoreBackend backend =
          ControllableSecureStoreBackend();
      final SecureStore secureStore = SecureStore(backend: backend);
      ...
      expect(
        (await aStore.loadNeuralHistory()).single.reasoning,
        contains('Recorded from a completed task occurrence'),
      );
      ...
      expect(
        (await _store('B', secureStore).loadNeuralHistory()).single.task,
        'shared-task',
      );
      expect((await aStore.loadNeuralHistory()).single.task, 'shared-task');
  ...
  test(
    'real trajectory follows the current scoped neural prediction',
    () async {
      final ControllableSecureStoreBackend backend =
          ControllableSecureStoreBackend();
      final SecureStore secureStore = SecureStore(backend: backend);
      ...
      container.read(_scopeProvider.notifier).setAuthenticated('A');
      final aTrajectory = await _trajectoryFor(
        container,
        expectsPrediction: true,
      );
      ...
      container.read(_scopeProvider.notifier).setSignedOut();
      expect(
        (await _trajectoryFor(
          container,
          expectsPrediction: false,
        )).predictionTitle,
        isNull,
      );
      container.read(_scopeProvider.notifier).setAuthenticated('B');
      final bTrajectory = await _trajectoryFor(
        container,
        expectsPrediction: true,
      );
      ...
      final cTrajectory = await _trajectoryFor(
        container,
        expectsPrediction: true,
      );
...
ProviderContainer _container(
  ...
  return ProviderContainer(
    overrides: [
      ...
      if (hiveStore != null) hiveStoreProvider.overrideWithValue(hiveStore),
...
Future<void> _completeTaskForAccount({
  ...
  final RealHiveTestFixture hiveFixture = await RealHiveTestFixture.create();
  ...
  final ProviderContainer container = _container(
    ...
    hiveStore: hiveFixture.hiveStore,
  );
...
ProviderContainer _trajectoryContainer(SecureStore secureStore) {
  return ProviderContainer(
    overrides: [
      ...
      learningMetricsProvider.overrideWith(
        (Ref ref) => _metricsFor(ref.watch(_scopeProvider)),
      ),
...
Future<TrajectorySummaryView> _trajectoryFor(
  ...
  final Completer<TrajectorySummaryView> result =
      Completer<TrajectorySummaryView>();
  late ProviderSubscription<TrajectorySummaryView> subscription;
  subscription = container.listen(trajectorySummaryProvider, (
    _,
    TrajectorySummaryView next,
  ) {
    if ((expectsPrediction
            ? next.hasPrediction
            : next.predictionTitle == null) &&
        !result.isCompleted) {
...
class _TrajectoryProfile extends ProfileController {
  ...
  ProfileState build() {
    final scope = ref.watch(_scopeProvider);
    return ProfileState(
      name: scope.isAuthenticated ? '${scope.rawUserId}_PROFILE' : 'signed_out',
      level: switch (scope.rawUserId) {
        'A' => 4,
        'B' => 2,
        'C' => 6,
        _ => 1,
      },
      profileReady: scope.isAuthenticated,
    );
  }
}
...
class _TrajectoryLearning extends LearningController {
  @override
  LearningState build() => LearningState(
    completed: switch (ref.watch(_scopeProvider).rawUserId) {
      'A' => 5,
      'B' => 2,
      'C' => 7,
      _ => 0,
...
class _TrajectorySiState extends SIStateController {
  ...
  SIState build() => SIState(
    energy: switch (ref.watch(_scopeProvider).rawUserId) {
      'A' => .9,
      'B' => .4,
      'C' => .6,
      _ => .5,
    ...
    fatigue: .2,
    completedToday: ref.watch(_scopeProvider).isAuthenticated ? 1 : 0,
  );
}

LearningMetrics _metricsFor(AccountStorageScope scope) =>
    switch (scope.rawUserId) {
      'A' => const LearningMetrics(
        completionRate: .9,
        momentum: .8,
        adaptability: .7,
      ),
      'B' => const LearningMetrics(
        completionRate: .3,
        momentum: .2,
        adaptability: .4,
      ),
      'C' => const LearningMetrics(
        completionRate: .8,
        momentum: .6,
        adaptability: .5,
      ),
      _ => const LearningMetrics(
        completionRate: 0,
        momentum: 0,
        adaptability: 0,
      ),
...
NeuralEntry _entryFor(String task, double quality, String reasoning) =>
    NeuralEntry(
      task: task,
      reasoning: reasoning,
      confidence: quality,

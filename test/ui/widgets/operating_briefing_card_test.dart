...
void main() {
  testWidgets('briefing visibly answers all four operating questions', (
    WidgetTester tester,
  ) async {
    ...
    final OperatingBriefing briefing = OperatingBriefing(
      snapshot: snapshot,
      ...
      MaterialApp(
        home: Scaffold(
          body: SingleChildScrollView(
            child: OperatingBriefingCard(
              briefing: briefing,
              topRisk: 'One overdue task',
              recentProgress: 'One task completed since the last briefing.',
              onAction: () {},
            ),
          ),
        ),
    ...
    expect(
      find.text('One task completed since the last briefing.'),
      findsOneWidget,
    );

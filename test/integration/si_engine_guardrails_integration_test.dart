...
void main() {
  group('SI engine guardrails', () {
    test(
      'selectResponseCandidate penalizes repeated wording using summaries',
      () {
        final SIResponseSelection selection = selectResponseCandidate(
          candidates: const <SIResponseCandidate>[
            ...
            SIResponseCandidate(
              message:
                  'Take five minutes to plan the next action for task beta.',
    ...
    test(
      'validateSIResponseDecision blocks ungrounded external-data/task-complete claims',
      () {
        ...
        const SIResponseCandidate candidate = SIResponseCandidate(
          message:
              'I performed an internet search and all tasks complete, so you are done.',
        ...
        expect(
          decision.violations,
          contains('external_data_claim_not_grounded'),
        );
        expect(
          decision.violations,
          contains('task_completion_claim_not_grounded'),
        );
    ...
    test('policy gate rejects unsupported AI-meta phrasing', () {
      final bool accepted = isPolicyAcceptableResponse(

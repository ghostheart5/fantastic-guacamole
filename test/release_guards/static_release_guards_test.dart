  ...
  group('static release guards', () {
    test('lib does not contain forbidden GitHub login strings', () {
      ...
      for (final file in files) {
        if (file.path.endsWith('.dart') || file.path.endsWith('.md') || file.path.endsWith('.txt')) {
      ...
      expect(matches, isEmpty, reason: 'Unexpected GitHub login strings: $matches');
    });
    ...
    test('lib does not contain TODO/FIXME/HACK in critical auth/storage/payment files', () {
      final files = <File>[
      ...
      expect(offenders, isEmpty, reason: 'Critical files contain TODO/FIXME/HACK: $offenders');
    });
    ...
    test('pubspec assets referenced by lib exist on disk', () {
      ...
      final assetMatches = RegExp(r'^\s*-\s+([A-Za-z0-9_./-]+)', multiLine: true)
          .allMatches(pubspec)

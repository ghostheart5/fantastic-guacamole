...
void main() {
  group('test inventory', () {
    test('feature folders have matching tests or documented reasons', () {
      final featureDirs = Directory('lib/features').listSync(recursive: true).whereType<Directory>();
      ...
      expect(reasonFile.existsSync(), isTrue, reason: 'Expected documented reasons file to exist.');
    });

    test('no empty test files', () {
      final testFiles = Directory('test').listSync(recursive: true).whereType<File>().where((file) => file.path.endsWith('.dart')).toList();

...
void main() {
  group('Test quality guard', () {
    test('dart tests do not contain placeholder markers or empty test bodies', () {
      ...
      for (final File file in SourceTestUtils.dartFilesUnder('test')) {
        ...
        final bool hasBannedPattern =
            bannedPatterns.any((RegExp pattern) => pattern.hasMatch(text));
        if (hasBannedLiteral || hasBannedPattern) {
          offenders.add(path);
        }
      }

      expect(offenders, isEmpty, reason: 'Low-quality or placeholder tests found: $offenders');
    });

    test('each dart test file contains a real assertion or widget verification path', () {
      final List<String> offenders = <String>[];

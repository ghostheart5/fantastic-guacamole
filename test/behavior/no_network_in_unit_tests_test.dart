...
void main() {
  group('No network in unit/behavior/architecture tests', () {
    test(
      'test suites avoid uncontrolled live network/platform dependencies',
      () {
        ...
        for (final File file in files) {
          ...
          final bool hasBanned = banned.any(
            (String token) => lower.contains(token.toLowerCase()),
          );
          final bool platformChannelWithoutMock =
              lower.contains('methodchannel(') &&
              !lower.contains('setmockmethodcallhandler');
        ...
        expect(
          ...
          isEmpty,
          reason:
              'Uncontrolled network/platform dependencies found: $offenders',
        );
      },
    );
  });

...
void main() {
  group('Performance static contract', () {
    test(
      'build methods do not perform heavy sync IO or storage init calls',
      () {
        ...
        expect(
          offenders,
          isEmpty,
          reason: 'Heavy operations found in build methods: $offenders',
        );
      },
    );

    test(
      'timer, stream subscription, and animation lifecycles include cleanup',
      () {
        final List<String> offenders = <String>[];
        ...
        for (final File file in SourceTestUtils.dartFilesUnder('lib')) {
          ...
          final bool isStatefulOwner =
              text.contains('extends State<') ||
              text.contains('with SingleTickerProviderStateMixin');
          if (!isStatefulOwner) {
            continue;
          }
          final bool hasTimer =
              text.contains('Timer.periodic(') || text.contains('Timer(');
          ...
          final bool hasCleanup =
              text.contains('.cancel();') || text.contains('.dispose();');

          if ((hasTimer || hasStreamSub || hasAnimation) &&
        ...
        expect(
          offenders,
          isEmpty,
          reason: 'Lifecycle cleanup issues detected: $offenders',
        );
      },
    );
  });

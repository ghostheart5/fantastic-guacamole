...
void main() {
  group('Source parse integrity', () {
    test('all lib dart files parse as strict UTF-8 and avoid mojibake markers', () {
      ...
      expect(offenders, isEmpty, reason: 'UTF-8 or mojibake issues detected: $offenders');
    });
    ...
    test('lib has no merge markers backup imports or test imports', () {
      ...
      for (final File file in allFiles) {
        final String normalized = SourceTestUtils.normalizePath(file.path).toLowerCase();
        ...
        if (RegExp(r'''import\s+['"][^'"]+\.bak[^'"]*['"]''').hasMatch(lowerText) ||
            lowerText.contains('package:flutter_test')) {
          offenders.add(normalized);
        }
      }

      expect(offenders, isEmpty, reason: 'Source integrity violations: $offenders');
    });
  });
}

...
void main() {
  ...
  group('Service source behavior', () {
    test('service files expose real methods and are not empty shells', () {
      ...
      final RegExp methodPattern = RegExp(r'\b[a-zA-Z_]\w*\s*\([^;{}]*\)\s*(?:async\s*)?(?:=>|\{|;)');
      ...
      for (final File file in SourceTestUtils.dartFilesUnder('lib')) {
        ...
        final bool hasGetter = RegExp(r'\bget\s+[a-zA-Z_]\w*\s*=>').hasMatch(text);
      ...
      expect(offenders, isEmpty, reason: 'Service files with insufficient behavior: $offenders');
    });

    test('service files avoid importing screens/widgets unless explicitly UI services', () {
      final List<String> offenders = <String>[];
      ...
      for (final File file in SourceTestUtils.dartFilesUnder('lib')) {
        ...
        final bool importsUi = text.contains('/screen') || text.contains('/page') || text.contains('/widget');
        final bool explicitUiService = lowerPath.contains('/ui/') || lowerPath.contains('/presentation/');
        if (importsUi && !explicitUiService) {
          offenders.add(path);
        }
      }

      expect(offenders, isEmpty, reason: 'Non-UI services importing UI layers: $offenders');
    });

    test('auth and storage service paths include expected operational verbs when present', () {
      final String all = SourceTestUtils.readAllConcatenated('lib').toLowerCase();

      final bool authExists = all.contains('auth_service') || all.contains('authservice');
      if (authExists) {
      ...
      final bool storageExists = all.contains('hive_service') || all.contains('shared_prefs_service');
      if (storageExists) {

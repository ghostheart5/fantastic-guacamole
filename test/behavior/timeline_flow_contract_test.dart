...
void main() {
  group('Timeline flow contract', () {
    ...
    test('Timeline feature includes history/activity/event data paths', () {
      final String timeline = SourceTestUtils.readAllConcatenated(
        'lib/features/timeline',
      ).toLowerCase();
      expect(timeline.contains('event'), isTrue);
      expect(
        timeline.contains('history') || timeline.contains('activity'),
        isTrue,
      );
    });
    ...
    test('Timeline is sole explicit history surface in ui screen naming', () {
      ...
      for (final File file in SourceTestUtils.dartFilesUnder('lib/features')) {
        ...
        final bool historyScreen =
            path.contains('history') && path.contains('screen');
      ...
      expect(
        offenders,
        isEmpty,
        reason: 'History surface appears outside timeline feature: $offenders',
      );
    });

    test('Timeline wiring references provider or repository contracts', () {
      final String timelineScreen = SourceTestUtils.readText(

...
void main() {
  ...
  test('provider files should contain real provider declarations', () {
    final files = dartFilesUnder('lib').where((file) {
    ...
    expect(
      ...
      isEmpty,
      reason:
          'Files named provider do not appear to declare providers: $suspicious',
    );
  });

  test(
    'repository files should expose load save fetch create update or delete behavior',
    () {
      final files = dartFilesUnder('lib').where((file) {
        final String path = file.path.toLowerCase().replaceAll('\\', '/');
      ...
      expect(
        ...
        isEmpty,
        reason:
            'Repository files missing obvious repository behavior: $suspicious',
      );
    },
  );
  ...
  test('controllers should expose action methods and not be empty shells', () {
    ...
    for (final file in files) {
      ...
      final methodLikeCount = RegExp(
        r'\b(?:Future<[^>]+>|void|bool|int|String|double|[A-Z][A-Za-z0-9_<>, ?]*)\s+[a-zA-Z_]\w*\s*\(',

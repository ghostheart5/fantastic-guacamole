...
class _TaskRepository implements ITaskRepository {
  _TaskRepository(TaskEntity task) : _tasks = <String, TaskEntity>{task.id: task};
...
void main() {
  test('completing twice cannot grant progress twice', () async {
    for (final int seed in phase10Seeds) {
      ...
      expect(progression.value.xp, 10, reason: 'seed=' + seed.toString());
      expect(progression.saves, 1, reason: 'seed=' + seed.toString());
      expect(tasks.saves, 1, reason: 'seed=' + seed.toString());
    }
  });

  test('fixed seeds preserve task, goal, habit, routine, note and Timeline data', () {
    for (final int seed in phase10Seeds) {
      final DeterministicGenerator g = DeterministicGenerator(seed);
      ...
      final String reason = 'seed=' + seed.toString();
      ...
      expect(taskRoundTrip.energyRequired, task.energyRequired, reason: reason);
      expect(taskRoundTrip.recurrenceRule, task.recurrenceRule, reason: reason);
      ...
      final GoalEntity archivedGoal = completedGoal.archive(created.add(const Duration(days: 1)));
      ...
      expect(HabitEntity.fromJson(habit.toJson()).title, habit.title, reason: reason);
      ...
      expect(RoutineEntity.fromJson(routine.toJson()).stepTaskIds, <String>[task.id], reason: reason);
      ...
      expect(NoteEntity.fromJson(note.toJson()).toTaskEntity().title, title, reason: reason);

      final List<TimelineEventEntity> ordered = List<TimelineEventEntity>.generate(
        12,
        (int index) => TimelineEventEntity(
          id: 'timeline-' + seed.toString() + '-' + index.toString(),
          type: TimelineEventType.task,
          title: title,
          detail: 'detail-' + index.toString(),
          timestamp: created.add(Duration(minutes: index)),
        ),
      )..sort((TimelineEventEntity a, TimelineEventEntity b) => a.timestamp.compareTo(b.timestamp));
      for (int index = 1; index < ordered.length; index++) {
        expect(ordered[index - 1].timestamp.compareTo(ordered[index].timestamp), lessThanOrEqualTo(0));
      }
      expect(TimelineEventEntity.fromJson(ordered.last.toJson()).id, ordered.last.id, reason: reason);
    }
  });

  test('malformed task payloads fail closed instead of becoming a lifecycle item', () {
    expect(

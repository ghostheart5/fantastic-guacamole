...
void main() {
  test('fixed XP inputs preserve progression boundaries and monotonic levels', () {
    for (final int seed in phase10Seeds) {
      ...
      for (int index = 0; index < 80; index++) {
        ...
        final ProgressionEntity progression = ProgressionEntity(xp: xp).addXp(0);
        expect(progression.level, greaterThanOrEqualTo(previousLevel));
        expect(ProgressionPolicy.xpForLevel(progression.level), lessThanOrEqualTo(xp));
  ...
  test('trajectory simulations stay finite and bounded for fixed generated signals', () {
    for (final int seed in phase10Seeds) {
      final DeterministicGenerator g = DeterministicGenerator(seed);
      ...
      expect(results, isNotEmpty, reason: 'seed=' + seed.toString());

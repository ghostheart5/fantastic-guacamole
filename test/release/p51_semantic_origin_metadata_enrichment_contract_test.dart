...
void main() {
  group('P5-1 Wave 2 semantic-origin metadata enrichment contract', () {
    test(
      'creator provider routes semantic actionSource values by requested kind',
      () {
    ...
    test(
      'task provider emits semantic creation reflection for routine and note',
      () {
        ...
        expect(
          text.contains("'note' => '\${task.title} note added to trajectory.'"),
          isTrue,
        );
      },
    );

...
void main() {
  group('Timeline release protection', () {
    test('timeline source exists and carries activity history semantics', () {
      ...
      final String text = SourceTestUtils.readAllConcatenated('lib/features/timeline').toLowerCase();
      expect(text.contains('timeline'), isTrue);
      expect(text.contains('history') || text.contains('activity') || text.contains('event'), isTrue);
    });

    test('duplicate generic history screens are not introduced outside timeline', () {
      ...
      expect(offenders, isEmpty, reason: 'History screen drift detected outside timeline: $offenders');
    });

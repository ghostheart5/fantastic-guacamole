...
void main() {
  group('High-value flow coverage contract', () {
    test('onboarding remains a 2-slide setup with expected messaging', () {
      ...
      expect(
        text.contains('Choose when they happen, then view everything on your Timeline.'),
        isTrue,
      );
      expect(
      ...
      expect(
        text.contains('SI Console helps you understand your goals, progress, and planning signals.'),
        isTrue,
      );
    });

    test('creator setup keeps create-to-timeline review loop', () {
      final File creatorFile = File('lib/features/creator/ui/creator_screen.dart');
      expect(creatorFile.existsSync(), isTrue);

      final String text = SourceTestUtils.readText(creatorFile);

      expect(text.contains('final bool shouldAutoOpenTimeline = !ref.read('), isTrue);
      expect(text.contains("'First item created. Reviewing it on your timeline...'"), isTrue);
      expect(text.contains('label: \'REVIEW TIMELINE\''), isTrue);
      expect(text.contains('ref.read(appFlowProvider.notifier).toTimeline();'), isTrue);
      expect(
        text.contains('After saving, ChronoSpark will show it on your Timeline.'),
        isTrue,
      );
    });

    test('smart planner follow-up keeps input on timeout/error and clears on success only', () {
      final File smartCoachFile =
          File('lib/features/home/ui/smart_coach_screen.dart');
    ...
    test('settings audio wiring initializes once and updates through setting listeners', () {
      final File settingsFile = File('lib/features/settings/ui/settings_screen.dart');
      expect(settingsFile.existsSync(), isTrue);

      final String text = SourceTestUtils.readText(settingsFile);

      expect(text.contains('WidgetsBinding.instance.addPostFrameCallback((_) {'), isTrue);
      expect(text.contains('_syncAudioSettings();'), isTrue);
      expect(text.contains('ref.listen<bool>(\n      soundEnabledProvider,'), isTrue);
      expect(text.contains('ref.listen<bool>(\n      advancedAudioProfileEnabledProvider,'), isTrue);
      expect(text.contains('ref.listen<bool>(\n      hapticFeedbackEnabledProvider,'), isTrue);
    });

    test('si console keeps inline degradation banner when messages already exist', () {
      final File siConsoleFile =
      ...
      expect(text.contains('(consoleError != null && _messages.isEmpty)'), isTrue);
      expect(text.contains('if (consoleError != null)'), isTrue);
      expect(text.contains('SI context is limited right now.'), isTrue);
      expect(text.contains('Some intelligence data could not refresh.'), isTrue);
    });
  });
}

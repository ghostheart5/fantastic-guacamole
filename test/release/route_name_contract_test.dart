...
void main() {
  group('Route name release contract', () {
    test('route path constants include core destinations and avoid internal screen drift', () {
    ...
    test('route constant values are not duplicated inconsistently', () {
      final File file = File('lib/app/router/route_paths.dart');
      final String text = SourceTestUtils.readText(file);
      final Iterable<Match> matches = RegExp(r"static const [a-zA-Z0-9_]+ = '([^']+)';").allMatches(text);
      ...
      expect(suspicious, isEmpty, reason: 'Suspiciously duplicated route constants: $suspicious');
    });
  });
}

...
void main() {
  group('P1-3 monetization stack normalization contract', () {
    ...
    test(
      'monetization guard utilities depend on feature providers, not legacy providers',
      () {
        final File file = File(
        ...
        expect(
          text.contains('Future<CreditConsumeResult> consumeCredits('),
          isTrue,
        );
    ...
    test(
      'paywall and plan comparison UI surfaces no longer import legacy providers',
      () {
        ...
        expect(
          paywallText.contains(
            "import 'package:fantastic_guacamole/features/monetization/providers/monetization_providers.dart';",
          ),
          isFalse,
        );
        expect(
          planComparisonText.contains(
            "import 'package:fantastic_guacamole/features/monetization/providers/monetization_providers.dart';",
          ),
          isFalse,
        );
        expect(
          featureProvidersText.contains(
            'final paywallProvider = FutureProvider<PaywallContent>(',
          ),
          isTrue,
        );
        expect(
          featureProvidersText.contains('final paywallPromptProvider ='),
          isTrue,
        );
      },
    );

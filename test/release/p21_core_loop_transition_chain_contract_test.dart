...
void main() {
  group('P2-1 core-loop transition chain contract', () {
    test('create-to-timeline action markers remain wired across task mutations', () {
    ...
    test('completion event write path remains persistence-first then invalidation', () {
      final File taskProviderFile = File('lib/state/providers/task_provider.dart');
      expect(taskProviderFile.existsSync(), isTrue);
    ...
    test('event-bus fan-out continues to invalidate completion events for task and goal lifecycles', () {
      final File eventBusFile = File('lib/state/providers/event_bus_provider.dart');
      expect(eventBusFile.existsSync(), isTrue);

      final String text = SourceTestUtils.readText(eventBusFile);

      final int taskListenerIndex = text.indexOf('bus.on<TaskLifecycleEvent>().listen');
      final int goalListenerIndex = text.indexOf('bus.on<GoalLifecycleEvent>().listen');

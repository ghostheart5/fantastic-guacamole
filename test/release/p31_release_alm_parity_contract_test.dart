...
void main() {
  group('P3-1 release ALM parity contract', () {
    test('manual release preflight workflow supports production staging and tester profiles', () {
      final File preflightWorkflow = File('.github/workflows/release-preflight.yml');
    ...
    test('android release workflow keeps security and production guardrails', () {
      final File androidReleaseWorkflow = File('.github/workflows/android-release.yml');
      ...
      expect(text.contains('CHRONOSPARK_ENABLE_TESTER_FULL_ACCESS=false'), isTrue);
    });

    test('repo still contains declared release workflows from ALM audit scope', () {
      ...
      expect(missing, isEmpty, reason: 'Missing ALM workflow files: $missing');
    });
  });
}

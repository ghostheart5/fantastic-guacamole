...
void main() {
  group('Audio speech release contract', () {
    test('audio and speech dependencies have source boundaries', () {
      ...
      final bool hasAudioDeps =
          pubspec.contains('audio_session') ||
          pubspec.contains('just_audio') ||
          pubspec.contains('audioplayers');
      final bool hasSpeechDeps =
          pubspec.contains('speech_to_text') || pubspec.contains('flutter_tts');
      ...
      if (hasAudioDeps) {
        expect(
          libText.contains('audio_session') ||
              libText.contains('audioservice') ||
              libText.contains('voice_service'),
          isTrue,
        );
      }
      ...
      if (hasSpeechDeps) {
        expect(
          libText.contains('speech_to_text') ||
              libText.contains('flutter_tts') ||
              libText.contains('voice'),
          isTrue,
        );
        expect(
          ...
          isTrue,
          reason:
              'Speech/TTS deps found but permission rationale path not found.',
        );
      }
    });
    ...
    test('widgets do not create uncancelled audio or speech loops in build', () {
      ...
      for (final File file in SourceTestUtils.dartFilesUnder('lib')) {
        ...
        if (buildBody.contains('speech_to_text') ||
            buildBody.contains('flutter_tts') ||
            buildBody.contains('audio_player') ||
            buildBody.contains('justaudio')) {
          offenders.add(path);
        }
      }
      ...
      expect(
        ...
        isEmpty,
        reason:
            'Audio/speech objects should not be instantiated in build: $offenders',
      );
    });
  });
}

...
void main() {
  group('Google Play readiness contract', () {
    test('android manifest and sdk configuration are present', () {
      ...
      expect(gradleText.contains('minsdk') || gradleText.contains('minsdkversion'), isTrue);
      expect(gradleText.contains('targetsdk') || gradleText.contains('targetsdkversion'), isTrue);
    });
    ...
    test('release signing secrets are not committed in obvious plaintext', () {
      ...
      for (final File file in SourceTestUtils.filesUnder('android')) {
        ...
        if (!path.endsWith('.gradle') && !path.endsWith('.kts') && !path.endsWith('.properties')) {
          continue;
        }
        ...
        final bool hasPlainPasswordInProperties = isProperties &&
            (RegExp(r'^\s*keypassword\s*=\s*[^\s#].+$', multiLine: true)
                    .hasMatch(text) ||
                RegExp(r'^\s*storepassword\s*=\s*[^\s#].+$', multiLine: true)
                    .hasMatch(text));
      ...
      expect(offenders, isEmpty, reason: 'Potential plaintext release secrets in android config: $offenders');
    });

    test('release command or docs exist and debug-only shipping markers are absent', () {
      final bool hasReleaseScript = File('tools/run_full_test_audit.ps1').existsSync() ||

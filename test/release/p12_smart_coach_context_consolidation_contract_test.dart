...
void main() {
  group('P1-2 smart coach context consolidation contract', () {
    ...
    test('SI console context assembly uses shared summary and signal helpers', () {
      final File controllerFile = File('lib/state/controllers/ai_controller.dart');
      final File helpersFile = File('lib/state/controllers/ai_controller.helpers.dart');
      ...
      expect(helpersText.contains('List<String> summarizeTimelineTitles('), isTrue);
      expect(helpersText.contains('List<String> summarizeCompletionEvents('), isTrue);
      expect(helpersText.contains('List<String> summarizeRoutineNames('), isTrue);
      expect(helpersText.contains('List<String> summarizeScheduledTaskTitles('), isTrue);
      expect(helpersText.contains('Map<String, dynamic> buildSIConsoleChronosparkSignals('), isTrue);
      ...
      expect(controllerText.contains('summarizeScheduledTaskTitles('), isTrue);
      expect(controllerText.contains('buildSIConsoleChronosparkSignals('), isTrue);
    });
  });
}

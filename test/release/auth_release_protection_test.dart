...
void main() {
  group('Auth release protection', () {
    test('auth controller paths exist for restore signin signup signout', () {
      final File authController = File(
    ...
    test('auth source exposes error path and auth state surface', () {
      final String appText = SourceTestUtils.readAllConcatenated(
        'lib/features/auth',
      ).toLowerCase();
      expect(appText.contains('authstatus'), isTrue);
      expect(appText.contains('error'), isTrue);
      expect(
        appText.contains('watchsession') ||
            appText.contains('getcurrentsession'),
        isTrue,
      );
    });

    test(
      'auth source includes verify or reset and optional phone path wiring',
      () {
        final String authText = SourceTestUtils.readAllConcatenated(
          'lib/features/auth',
        ).toLowerCase();
        final String loginText = SourceTestUtils.readText(
          File('lib/features/auth/ui/login_screen.dart'),
        ).toLowerCase();

        expect(
          authText.contains('sendpasswordreset') || authText.contains('verify'),
          isTrue,
        );

        if (loginText.contains('phone')) {
          expect(
            authText.contains('phone'),
            isTrue,
            reason:
                'Phone appears in login UI but no phone path found in auth source.',
          );
        }
      },
    );
    ...
    test(
      'signup does not automatically resend verification and callback config is documented',
      () {
        ...
        final String env = SourceTestUtils.readText(
          File('lib/config/env.dart'),
        );
        final String envExample = SourceTestUtils.readText(
          File('.env.example'),
        );
        final String readme = SourceTestUtils.readText(
          File('README.md'),
        ...
        expect(
          env.contains('CHRONOSPARK_PASSWORD_RECOVERY_REDIRECT_URL'),
          isTrue,
        );
        expect(
          env.contains('productionAuthCallbackUrl'),
          isTrue,
        );
        expect(
          env.contains('defaultValue: customSchemeAuthCallbackUrl'),
          isTrue,
        );
        expect(
          envExample.contains(
            'CHRONOSPARK_OAUTH_REDIRECT_URL=chronospark://auth-callback',
          ),
          isTrue,
        );
        expect(
          envExample.contains(
            'CHRONOSPARK_PASSWORD_RECOVERY_REDIRECT_URL=chronospark://auth-callback',
          ),
          isTrue,
        );
        expect(readme.contains('Supabase Auth production setup'), isTrue);
        expect(
          readme.contains('SMTP provider or Send Email Auth Hook'),
          isTrue,
        );
    ...
    test('auth source does not contain hard-coded credentials', () {
      ...
      expect(
        offenders,
        isEmpty,
        reason: 'Auth files with possible hard-coded credentials: $offenders',
      );
    });
  });
}

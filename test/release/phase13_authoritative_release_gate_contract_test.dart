...
void main() {
  test('authoritative evidence template defines all ordered mandatory stages', () {
    final Map<String, dynamic> template = jsonDecode(
      File('tool/release_gate/phase13_evidence_template.json').readAsStringSync(),
    ) as Map<String, dynamic>;
    final List<dynamic> stages = template['stages'] as List<dynamic>;
    expect(stages, hasLength(19));
    expect((stages.first as Map<String, dynamic>)['name'], 'repositoryIntegrity');
    expect((stages[9] as Map<String, dynamic>)['name'], 'signedCandidateBuild');
    expect((stages[10] as Map<String, dynamic>)['name'], 'binaryHashGeneration');
    expect((stages.last as Map<String, dynamic>)['name'], 'independentReleaseApproval');
  });

  test('release workflows consume gated artifacts instead of rebuilding', () {
    final String android = File('.github/workflows/android-release.yml').readAsStringSync();
    final String web = File('.github/workflows/main.yml').readAsStringSync();
    expect(android, contains('resolve-authoritative-gate'));
    expect(web, contains('resolve-authoritative-gate'));
    expect(android, isNot(contains('flutter build appbundle')));
    expect(web, isNot(contains('flutter build web')));
  });

  test('manifest validator blocks dirty, mismatched, unsigned, or connected-chat evidence', () {
    final String source = File('scripts/authoritative_release_gate.ps1').readAsStringSync();

...
void main() {
  group('P4-1 routine/note first-class surface contract', () {
    test('creator saved kind enum exposes routine and note categories', () {
      ...
      expect(text.contains('enum CreatorSavedKind { task, goal, routine, note }'), isTrue);
    });

    test('create entry captures requested kind and returns distinct save kind', () {
      final File creatorProviderFile = File('lib/state/providers/creator_provider.dart');
      expect(creatorProviderFile.existsSync(), isTrue);
    ...
    test('creator screen preserves metrics recording for routine and note saves', () {
      final File creatorScreenFile = File('lib/features/creator/ui/creator_screen.dart');

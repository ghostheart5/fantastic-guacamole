...
void main() {
  ...
  test('old schema migrates to current schema', () async {
    ...
    expect(settings.get(StorageKeys.storageVersion), StorageMigration.latestVersion);
  ...
  test('unknown future version does not destroy data', () async {
    final settings = await Hive.openBox<dynamic>(StorageKeys.settings);
    final notifications = await Hive.openBox<dynamic>(StorageKeys.notifications);
    await settings.put(StorageKeys.storageVersion, StorageMigration.latestVersion + 9);
    await notifications.put('n1', 'keep-this');

    await StorageMigration.run();

    expect(settings.get(StorageKeys.storageVersion), StorageMigration.latestVersion + 9);
    expect(notifications.get('n1'), 'keep-this');
  });

  test('migration preserves tasks logs settings and subscription state', () async {
    final settings = await Hive.openBox<dynamic>(StorageKeys.settings);

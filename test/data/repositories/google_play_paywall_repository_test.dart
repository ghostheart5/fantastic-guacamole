...
void main() {
  ...
  test('getAvailablePlans maps live Google Play prices when products resolve', () async {
    ...
    expect(plans.firstWhere((plan) => plan.id == 'monthly').priceLabel, '4.99');
    expect(plans.firstWhere((plan) => plan.id == 'monthly').isAvailable, isTrue);
    expect(plans.firstWhere((plan) => plan.id == 'annual').isAvailable, isFalse);

    repository.dispose();
  });

  test('purchase stream errors are tolerated without crashing repository', () async {
  ...
  test('getAvailablePlans falls back to static plans when billing lookup throws', () async {
    final _FakeBillingClient billing = _FakeBillingClient(
      productResponse: ProductDetailsResponse(
        productDetails: const <ProductDetails>[],
        notFoundIDs: const <String>[],
    ...
    final plans = await Logger.withMutedErrors(() => repository.getAvailablePlans());
  ...
  test('startSubscription throws when product is missing in Google Play', () async {
    final _FakeBillingClient billing = _FakeBillingClient(
      productResponse: ProductDetailsResponse(
        productDetails: const <ProductDetails>[],
  ...
  test('startSubscription resolves active state after verified purchase update', () async {
    final StreamController<List<PurchaseDetails>> controller =
        StreamController<List<PurchaseDetails>>.broadcast();
    final _FakeBillingClient billing = _FakeBillingClient(
      purchaseStreamController: controller,
    ...
    final GooglePlayPaywallRepository repository = GooglePlayPaywallRepository(
      ...
      httpClient: MockClient((http.Request request) async {
        expect(request.url.toString(), 'https://api.chronospark.app/verify');
    ...
    final SubscriptionState state = await repository.startSubscription('monthly');
  ...
  test('startSubscription returns verification_failed when receipt check fails', () async {
    final StreamController<List<PurchaseDetails>> controller =
        StreamController<List<PurchaseDetails>>.broadcast();
    final _FakeBillingClient billing = _FakeBillingClient(
      purchaseStreamController: controller,
  ...
  test('restorePurchases resolves restored state from purchase stream', () async {
    final StreamController<List<PurchaseDetails>> controller =
        StreamController<List<PurchaseDetails>>.broadcast();
    final _FakeBillingClient billing = _FakeBillingClient(
      purchaseStreamController: controller,
  ...
  test('restorePurchases in testing mode unlocks using annual fallback', () async {
    final _FakeBillingClient billing = _FakeBillingClient(
      productResponse: ProductDetailsResponse(
        productDetails: const <ProductDetails>[],
        notFoundIDs: const <String>[],
  ...
  test(
    ...
    () async {
      ...
      await Logger.withMutedErrors(
        () => expectLater(() => repository.startSubscription('monthly'), throwsA(isA<IAPError>())),
      );
      expect(billing.completePurchaseCalls, 1);
  ...
  test('restored purchase with unknown product id falls back to monthly plan id', () async {
    final StreamController<List<PurchaseDetails>> controller =
        StreamController<List<PurchaseDetails>>.broadcast();
    final _FakeBillingClient billing = _FakeBillingClient(
      purchaseStreamController: controller,
  ...
  test('missing receipt endpoint disables plans and blocks purchase/restore', () async {
    final StreamController<List<PurchaseDetails>> controller =
        StreamController<List<PurchaseDetails>>.broadcast();
    final _FakeBillingClient billing = _FakeBillingClient(
      purchaseStreamController: controller,
    ...
    expect(plans.every((PaywallPlan plan) => plan.isAvailable == false), isTrue);

    await expectLater(() => repository.startSubscription('monthly'), throwsA(isA<StateError>()));

    await expectLater(() => repository.restorePurchases(), throwsA(isA<StateError>()));
  ...
  test('receipt verification treats non-200 and invalid JSON as locked', () async {
    final StreamController<List<PurchaseDetails>> controller =
        StreamController<List<PurchaseDetails>>.broadcast();
  ...
  test('cancelSubscription updates persisted user state', () async {
    SharedPreferences.setMockInitialValues(<String, Object>{
      'paywall_subscription_state_v1': jsonEncode(<String, dynamic>{
        ...
        'status': 'active',
        'planId': 'monthly',
        'renewalDate': DateTime.now().add(const Duration(days: 3)).toIso8601String(),
      }),
    });
    final _FakeBillingClient billing = _FakeBillingClient(
      productResponse: ProductDetailsResponse(
    ...
    final Map<String, dynamic> persisted =
        jsonDecode(prefs.getString('paywall_subscription_state_v1')!) as Map<String, dynamic>;
  ...
  test('expired persisted state loads as locked while preserving renewal metadata', () async {
  ...
  test('cancelSubscription survives persistence loader failures', () async {
    final GooglePlayPaywallRepository repository = await Logger.withMutedErrors(() async {
      return GooglePlayPaywallRepository(
        ...
        paywallTestingModeOverride: false,
        sharedPreferencesLoader: () => Future<SharedPreferences>.error(Exception('prefs failed')),
      );
    });
  ...
  test('testing mode unlocks paywall without billing calls', () async {
    ...
    final paywall = await repository.getPaywallConfig();
    final entitlement = await repository.checkEntitlement(featureId: 'premium');
    final SubscriptionState state = await repository.startSubscription('annual');

...
void main() {
  ...
  for (final action in ['retry', 'restore']) {
    for (final validProof in [true, false]) {
      test(
        'server completed stale pending $action requires exact proof ($validProof)',
        () async {
          final client = await _authorityClient(
            (_) async => http.Response(
              '[]',
            ...
            httpClient: MockClient((request) async {
              ...
              return http.Response(
                jsonEncode({
                  ...
                  'consumed': true,
                  'testPurchase': true,
                  'productId': validProof ? product : 'wrong-product',
                  'creditsGranted': 100,
                }),
                200,
              );
          ...
          try {
            ...
            expect(
              result.status,
              validProof ? 'credits_added' : 'purchase_pending',
            );
            expect(await store.readString(key), validProof ? isNull : owner);
            expect(billing.buyCalls, 0);
            expect(billing.completePurchaseCalls, 0);
  ...
  test(
    'absent Play inventory releases a stale public credit checkout guard',
    () async {
      final controller = StreamController<List<PurchaseDetails>>.broadcast();
      final client = await _authorityClient(
        (_) async => http.Response('[]', 200),
      ...
      final pendingPurchase = PurchaseDetails(
        purchaseID: 'later-canceled',
        productID: 'chronospark_credits_100',
        verificationData: PurchaseVerificationData(
          localVerificationData: '',
          serverVerificationData: 'later-canceled-token',
          source: 'google_play',
        ),
        transactionDate: '1',
        status: PurchaseStatus.pending,
      );
      final inventory = <PurchaseDetails>[pendingPurchase];
      ...
      final billing = _FakeBillingClient(
        ...
        restoredPurchases: inventory,
        onRestorePurchases: () async {
          if (inventoryFails) throw StateError('Play inventory unavailable');
        },
        onBuyNonConsumable: (_) async {
          controller.add([pendingPurchase]);
          return true;
      ...
      expect(
        (await repository.startSubscription('credits_100')).status,
        'purchase_pending',
      );
      inventoryFails = true;
      expect(
      ...
      inventory.clear(); // Play no longer owns the canceled pending order.
      expect(
        (await repository.startSubscription('credits_100')).status,
        'purchase_pending',
      );
  ...
  for (final status in <String>[
    ...
    test('Play inventory replay returns $status instead of pending', () async {
      final controller = StreamController<List<PurchaseDetails>>.broadcast();
      final client = await _authorityClient(
        (_) async => http.Response('[]', 200),
      );
      final pendingPurchase = PurchaseDetails(
        purchaseID: 'offline-completion',
        productID: 'chronospark_credits_100',
        verificationData: PurchaseVerificationData(
          localVerificationData: '',
          serverVerificationData: 'offline-completion-token',
          source: 'google_play',
        ),
        transactionDate: '1',
        status: PurchaseStatus.pending,
  ...
  test(
    ...
    () async {
      ...
      final sb.SupabaseClient client = await _authorityClient((request) async {
        fail('Active local state should block before an authority request.');
      });
      final GooglePlayPaywallRepository repository =
          GooglePlayPaywallRepository(
            billingClient: billing,
  ...
  test(
    ...
    () async {
      ...
      final sb.SupabaseClient client = await _authorityClient((request) async {
        fail('Purchase persistence should not refresh account authority.');
      });
      final StreamController<List<PurchaseDetails>> controller =
          StreamController<List<PurchaseDetails>>.broadcast();
      final _FakeBillingClient billing = _FakeBillingClient(
      ...
      expect(restored.renewalDate!.isBefore(verifiedExpiry), isTrue);
      expect(
        restored.renewalDate!.difference(DateTime.now().toUtc()),
  ...
  for (final status in <PurchaseStatus>[
    ...
    test(
      'empty-product $status clears its owner before immediate retry',
      () async {
        final controller = StreamController<List<PurchaseDetails>>.broadcast();
        final client = await _authorityClient((request) async {
          fail('A failed purchase must not verify or grant access.');
        });
        final store = SecureStore(backend: InMemorySecureStoreBackend());
        var starts = 0;
        final billing = _FakeBillingClient(
        ...
        try {
          for (var attempt = 0; attempt < 2; attempt++) {
            final purchase = repository
                .startSubscription('monthly')
                .timeout(const Duration(seconds: 2));
            if (status == PurchaseStatus.error) {
              await Logger.withMutedErrors(
                () => expectLater(purchase, throwsA(isA<IAPError>())),
              );
            } else {
              final state = await purchase;
              expect(state.status, 'purchase_canceled');
              expect(state.isActive, isFalse);
            }
            expect(
  ...
  for (final scenario in <String>[
    ...
    test('empty-product callback preserves ownership for $scenario', () async {
      final controller = StreamController<List<PurchaseDetails>>.broadcast();
      final client = await _authorityClient(
      ...
      final billing = _FakeBillingClient(
        ...
        onBuyNonConsumable: (param) async => true,
      );
      final repository = GooglePlayPaywallRepository(
        billingClient: billing,
        paywallTestingModeOverride: false,
        ...
        secureStore: store,
        httpClient: MockClient((request) async {
          fail('An empty-product success must not verify a purchase.');
        }),
      );
      var completed = false;
      repository.startSubscription('monthly').then((_) {
        completed = true;
      }).ignore();
      await pumpEventQueue(times: 10);
      if (scenario == 'ambiguous') {
        repository.startSubscription('annual').ignore();
        await pumpEventQueue(times: 10);
      } else if (scenario == 'other account') {
        await client.auth.signInWithPassword(
          email: 'user-2@example.com',
          password: 'password',
        );
  ...
  test('cancelSubscription fails closed without account authority', () async {
    SharedPreferences.setMockInitialValues(<String, Object>{
      'paywall_subscription_state_v1': jsonEncode(<String, dynamic>{
        'isActive': true,
        'status': 'active',
  ...
  test('loads persisted active state and exposes entitlement', () async {
    final DateTime renewal = DateTime.now().add(const Duration(hours: 12));
    SharedPreferences.setMockInitialValues(<String, Object>{
      'paywall_subscription_state_v1.account.user-1':
          jsonEncode(<String, dynamic>{
            'isActive': true,
    ...
    final sb.SupabaseClient client = await _authorityClient((request) async {
      fail('Persisted-state reads should not refresh account authority.');
    });
    final GooglePlayPaywallRepository repository = GooglePlayPaywallRepository(
      billingClient: billing,
      paywallTestingModeOverride: false,
  ...
  test(
    'reads proven-owner legacy state without copying or deleting it',
    () async {
      ...
      expect(prefs.getString('paywall_subscription_state_v1'), legacyState);

...
class _Timeline implements ITimelineRepository {
  ...
  List<TimelineEventEntity> getEvents() =>
      List<TimelineEventEntity>.from(values);
  @override
  ...
  Future<void> removeEvent(String id) async =>
      values.removeWhere((TimelineEventEntity event) => event.id == id);
  @override
...
class _Queue implements SyncQueueStoreContract {
  ...
  @override
  Future<void> overwrite(List<SyncOperation> items) async {
    values
      ..clear()
      ..addAll(items);
  }
  ...
  @override
  Future<void> enqueue(SyncOperation item) async => values.add(item);
  @override
  Future<void> removeById(String id) async =>
      values.removeWhere((SyncOperation item) => item.operationId == id);
  @override
  Future<void> update(SyncOperation updated) async => overwrite(
    values
        .map(
          (SyncOperation item) =>
...
void main() {
  ...
  setUpAll(
    () => Hive.init(Directory.systemTemp.createTempSync('note-b2-').path),
  );

  test(
    'Note Timeline projection is deterministic, typed, and retry-safe',
    () async {
      ...
      await adapter.record(
        ...
        NoteTimelineMutation.archived,
      );
      expect(
        timeline.values.map((TimelineEventEntity event) => event.type),
        <TimelineEventType>[
      ...
      expect(
        timeline.values.first.id,
        'note:same-note:created:1786791600000000',
      );
      timeline.fail = true;
      await expectLater(
        adapter.record(
          note.copyWith(updatedAt: DateTime.utc(2026, 8, 15, 14)),
  ...
  test(
    'Note local persistence queues one account-owned canonical sync mutation and retries',
    () async {
      ...
      final SyncRunner failing = SyncRunner(
        queueStore: queue,
      ...
      final SyncRunner retry = SyncRunner(
        queueStore: queue,
        now: () => DateTime.utc(2026, 8, 16),
        applyFn: (_) async => SyncApplyResult.success(),
      );
      await retry.runOnce();
  ...
  test(
    'Note Sync keeps matching logical ids isolated by account and rejects signed-out dispatch',
    () async {
      final _Queue a = _Queue();
      final _Queue b = _Queue();
      final NoteEntity note = _note('SAME_ID');
      await SyncMutationDispatcher(queueStore: a, userId: 'A').enqueueUpsert(
        tableName: 'notes',
        recordId: note.id,
        payload: note.toJson(),
      );
      await SyncMutationDispatcher(queueStore: b, userId: 'B').enqueueUpsert(
        tableName: 'notes',
        recordId: note.id,
        payload: note.toJson(),
      );
  ...
  test(
    'Note Sync restores an A-owned retry after real-Hive runtime recreation and keeps B isolated',
    () async {
      ...
      final SyncMutationDispatcher firstDispatcher = SyncMutationDispatcher(
        queueStore: aQueue,
        userId: 'note-restart-a',
      );
  ...
  test(
    'retained Note dispatcher remains bound to A and cannot enqueue into B',
    () async {
      final RealHiveTestFixture fixture = await RealHiveTestFixture.create();
      addTearDown(fixture.dispose);
      final AccountStorageScope a = AccountStorageScope.authenticated(
      ...
      final SyncQueueStore aQueue = _queue(fixture, a);
      final SyncQueueStore bQueue = _queue(fixture, b);
      final SyncMutationDispatcher staleA = SyncMutationDispatcher(
        queueStore: aQueue,
        userId: 'note-stale-a',
      );
      _repository(
        fixture,
        b,
        SyncMutationDispatcher(queueStore: bQueue, userId: 'note-stale-b'),
      );
      expect(
  ...
  test(
    'real NotesRemoteGateway maps owned Note rows and SupabaseSyncExecutor dispatches only notes',
    () async {
  ...
  test(
    'SupabaseSyncExecutor uniformly catches non-retryable async gateway failures',
    () async {
      ...
      for (final String tableName in <String>[
        ...
        final SyncApplyResult result = await executor.apply(
          SyncOperation(
  ...
  test(
    'real NotesRemoteGateway soft-deletes a canonical Note through notes dispatch only',
    () async {
      final _GatewayTransport transport = _GatewayTransport();
      final SupabaseSyncExecutor executor = SupabaseSyncExecutor(
        tasksGateway: const TasksRemoteGateway(null),

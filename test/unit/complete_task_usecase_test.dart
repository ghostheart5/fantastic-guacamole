...
void main() {
  group('CompleteTask workflow', () {
    ...
    test(
      'completing recurring task creates next scheduled occurrence',
      () async {
        final _FakeTaskRepository repo = _FakeTaskRepository(
          seed: <TaskEntity>[
            TaskEntity(
    ...
    test(
      'completing recurring task early preserves cadence from scheduled time',
      () async {
        ...
        expect(next.scheduledFor, isNotNull);
        expect(
          next.scheduledFor!.difference(expected).inSeconds.abs(),
          lessThan(2),
        );
      },
    );
    ...
    test(
      'overdue recurring completion rolls forward to a future occurrence',
      () async {
        final DateTime now = DateTime.now();

...
Write-Host "Adding real Creator entry lists..." -ForegroundColor Cyan

$utf8NoBom = New-Object System.Text.UTF8Encoding($false)

$listsPath = ".\lib\features\creator\ui\widgets\creator_entry_lists.dart"
...
if (-not (Test-Path $workbenchPath)) {
  throw "Missing Creator workbench: $workbenchPath"
}

New-Item -ItemType Directory -Force -Path ".\lib\features\creator\ui\widgets" | Out-Null

Copy-Item $workbenchPath "$workbenchPath.bak_entry_lists" -Force
if (Test-Path $barrelPath) {
  Copy-Item $barrelPath "$barrelPath.bak_entry_lists" -Force
}
...
class CreatorEntryLists extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return tasksAsync.when(
      ...
      data: (tasks) {
        final List<TaskEntity> all = List<TaskEntity>.from(tasks);

        final List<TaskEntity> taskEntries = all
            .where((task) => !_isCreatorSpecialKind(task.kind))
            .toList();

        final List<TaskEntity> goals = all
            .where((task) => task.kind.trim().toLowerCase() == 'goal')
            .toList();

        final List<TaskEntity> milestones = all
            .where((task) => task.kind.trim().toLowerCase() == 'milestone')
            .toList();

        final List<TaskEntity> planItems = all
            .where((task) => task.kind.trim().toLowerCase() == 'plan')
            .toList();
...
class _CreatorEntryGroup extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final List<TaskEntity> visibleEntries = entries.take(4).toList();
              ...
              children: visibleEntries
                  .map(
                    (entry) => Padding(
                      ...
                      child: _CreatorEntryTile(entry: entry, accent: accent),
                    ),
                  )
                  .toList(),
            ),
...
[System.IO.File]::WriteAllText($listsPath, $listsContent, $utf8NoBom)

# Wire into workbench.
$workbench = Get-Content $workbenchPath -Raw

if ($workbench -notmatch "creator_entry_lists.dart") {
...
if ($workbench -notmatch "CreatorEntryLists") {
  $workbench = $workbench.Replace(
...
if ($barrel -notmatch "creator_entry_lists.dart") {
  $barrel = $barrel.TrimEnd() + "`r`nexport 'ui/widgets/creator_entry_lists.dart';`r`n"
...
Write-Host "Creator entry lists are now real." -ForegroundColor Green

...
for (const scoped of [false, true]) {
  Deno.test(`${scoped ? "license-scoped" : "normal"} durable refund claim survives an uncertain response without a second refund`, async () => {
    ...
    const provider = Deno.serve(
      ...
      async (req) => {
        ...
        if (url.pathname.endsWith(`/products/${productId}/tokens/${token}`)) {
          assert(
            scoped && req.method === "GET",
            "Unexpected license-test read",
          );
          return Response.json({
            purchaseType: 0,
            orderId,
            consumptionState: 0,
            purchaseState: providerState === "REFUNDED" ? 1 : 0,
          });
        }
        if (url.pathname.endsWith(":refund")) {
          ...
          const candidates = state?.candidates as Array<
    ...
    const input = {
      ...
      ...(scoped ? { testTokenHash: tokenHash } : {}),
    };
    try {
      ...
      const settled = await reconcilePublicCreditRefunds(input, transport);
      assert(
        (scoped ? settled === null : settled?.scanned === 0) &&
          refundPosts === 1,

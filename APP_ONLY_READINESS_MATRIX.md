...
| Golden visual regression | `LOCAL PASS / EXACT-COMMIT CI PENDING` | The Gate 1 working change executes five visually reviewed, exact image comparisons: Login at 320/500 and Nexus at 320/375/500. The harness loads Inter and Material Icons and disables supported animations instead of tolerating pixel drift. | Baseline source `6118ba6` contains zero image comparisons, and the working change has not received an exact-commit CI run. | Commit only with authorization, run CI at that exact SHA, and record the five-matcher guard plus normal comparison result. |
...
| Gate 1 reviewed golden comparison | `LOCAL PASS` | Two consecutive runs of `flutter test --no-pub test/features/auth/login_screen_golden_test.dart test/features/nexus/nexus_screen_golden_test.dart` each passed 15 tests in normal comparison mode, including all five visually reviewed images under the default exact comparator. This is uncommitted host evidence, not CI or device evidence. |
| Gate 1 golden assertion guard | `LOCAL PASS / CI CONFIGURED` | The guard found five named baselines and five literal matcher declarations across both golden test files; both CI and the update-goldens workflow invoke it before testing or regeneration. No exact-commit CI result exists yet. |
| Gate 1 exact-commit CI | `NOT_RUN` | Requires an authorized commit/push and a CI run whose recorded SHA equals that commit. |
...
- [x] All five golden baselines are accepted after visual review and pass two consecutive exact normal comparison runs.

...
| Priority 6 first-use context proof | `LOCAL PASS / EXIT GATE OPEN` | 65 focused Windows-host tests passed serially, covering unchanged first-value accessibility, account-scoped one-shot behavior, signed-out failure, consent-before-save, policy eligibility, decline-without-write, context invalidation, visible Nexus discovery, one-shot Settings opening, governance controls, exact Windows Nexus comparisons, semantics, and true 320x568/375x667 viewport captures at 200% text with scroll reachability. Targeted analysis and `git diff --check` passed. The moderated protocol records elapsed time and facilitator rescue from before first value through comprehension, save, rediscovery, and confirmed deletion. Human sessions and Linux/exact-commit evidence were not run. |

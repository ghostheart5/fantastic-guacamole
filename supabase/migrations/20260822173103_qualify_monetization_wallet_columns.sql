-- Keep the public RPC signature and authorization contract intact while making
-- every table-column reference unambiguous against its RETURNS TABLE fields.
...
begin
  ...
  if wallet_row.period_ends_at is not null and wallet_row.period_ends_at <= now() then
    ...
    set tier = case active_status.plan_id
        ...
        balance = wallet.bonus_balance + case active_status.plan_id
          when 'premium_monthly' then 250
          when 'premium_yearly' then 4000
          when 'lifetime' then 0
          else 20
  ...
  update public.monetization_wallets as wallet
  set bonus_balance = wallet.bonus_balance - bonus_used,
      allowance_remaining = greatest(wallet.allowance_remaining - allowance_used, 0),

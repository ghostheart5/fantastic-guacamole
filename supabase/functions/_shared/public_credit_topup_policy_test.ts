...
const approved = (name: string): string | undefined =>
  (required as readonly string[]).includes(name) ? "true" : undefined;

Deno.test("license-scoped refund worker cannot authorize public sales", () => {
  for (
    const flags of [
      { PUBLIC_CREDIT_REFUND_TEST_ONLY: "true" },
      { PUBLIC_CREDIT_REFUND_TEST_ONLY: "TRUE" },
      { PUBLIC_CREDIT_REFUND_TEST_TOKEN_HASH: "a".repeat(64) },
      {
        PUBLIC_CREDIT_REFUND_TEST_ONLY: "false",
        PUBLIC_CREDIT_REFUND_TEST_TOKEN_HASH: "",
      },
    ]
  ) {
    const overrides: Record<string, string | undefined> = flags;
    const policy = parsePublicCreditTopupPolicy((name) =>
      overrides[name] ?? approved(name)
    );
    if (publicCreditSaleEnabled(policy)) {
      throw new Error("scoped worker opened public sales");
    }
  }
  const normal = parsePublicCreditTopupPolicy((name) =>
    name === "PUBLIC_CREDIT_REFUND_TEST_ONLY" ? "false" : approved(name)
  );
  if (!publicCreditSaleEnabled(normal)) {
    throw new Error("normal reviewed worker was denied");
  }
});
...
Deno.test("every public sale approval must be an exact server affirmative", () => {
  for (const absent of required) {
    const policy = parsePublicCreditTopupPolicy((name) =>
      name === absent ? undefined : approved(name)
    );
    if (publicCreditSaleEnabled(policy)) {
  ...
  const malformed = parsePublicCreditTopupPolicy((name) =>
    name === "CHRONOSPARK_PUBLIC_BILLING_REVIEW_APPROVED"
      ? "TRUE"
      : approved(name)
  );
...
Deno.test("closing new sales cannot invalidate an already-paid receipt", () => {
  const active = parsePublicCreditTopupPolicy(approved);
  const closed = parsePublicCreditTopupPolicy(() => undefined);
...
Deno.test("reviewed public rollout still honors an internal client's test demand", () => {
  const policy = parsePublicCreditTopupPolicy(approved);
  if (!publicCreditSaleEnabled(policy)) {

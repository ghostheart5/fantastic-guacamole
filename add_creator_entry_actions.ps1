...
Write-Host "Adding real actions to Creator entry tiles..." -ForegroundColor Cyan
...
Copy-Item $file "$file.bak_actions" -Force
    ...
    return tasksAsync.when(
      ...
      data: (tasks) {
        ...
        final List<Task> milestones = all
            .where(
              (task) => (task.kind ?? '').trim().toLowerCase() == 'milestone',
            )
            .toList();
...
Write-Host "Formatting Creator entry actions..." -ForegroundColor Cyan
dart format $file
...
Write-Host "Creator entry actions are now real." -ForegroundColor Green

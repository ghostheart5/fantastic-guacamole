...
1. Send each prompt in Smart Planner.
...
5. Confirm conversation turn is persisted in planner message history.
...
| Follow-up quality | Planner Question asks for a decision, metric, or commitment. |
...
| --- | --- | --- |
| AI recommendation returns non-structured text | Inject or simulate unstructured output | Controller returns deterministic structured planning-guidance block. |
| AI provider throws exception | Simulate executeSmartPlannerQuery failure path | Controller catches error and returns deterministic local fallback response. |
| Crisis phrase in input | Prompt includes self-harm crisis terms | Crisis policy path is triggered and planning-guidance generation is bypassed. |

## Persistence Checks

| Check | Expected Result |
| --- | --- |
| Initial planning-guidance turn saved | One user and one assistant PlannerMessage persisted for planning request. |
| Follow-up turn saved | One user and one assistant PlannerMessage persisted for each follow-up. |
...
- Topic routing and deterministic response generation come from state/controllers/smart_planner_query_controller.dart.
- Conversation persistence path uses savePlannerMessageUseCaseProvider via PlannerMessage entity.

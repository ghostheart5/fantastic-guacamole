...
## Phase 8 Checkpoint - Billing Authority And Entitlement Truth

- Status: `BLOCKED_EXTERNAL`; launch verdict remains `NO-GO - NOT READY`.
- Containment: subscriptions, paid credit plans, external AI, and credit spending remain unreachable. Paid plans require the subscription, external-AI, credit-spending, provider-retention, and qualified-review gates to all be true; every gate remains false.
- Provider authority: Google Play line selection is deterministic and fails closed on ambiguous active products. Deferred replacement cancellation rechecks the linked predecessor. Subscriptions-center re-purchases use provider lineage, while active replacements remain distinct and a client with active authority cannot start a second plan purchase. Provider-confirmed hold recovery and hold-time repurchase restore one paid allowance through an explicit idempotent recovery cause. Active access requires provider state, expiry, product, account binding, acknowledgement, and ordering to agree. Refunded, revoked, expired, held, paused, pending, canceled, stale, old-token, and terminal replay states do not grant unproven access or credits.
- Account binding: purchase requests use a lowercase SHA-256 account fingerprint. Pending ownership survives restart in secure storage, cannot cross an app-account change, and is cleared by account cleanup. Missing provider identifiers are accepted only under an explicit legacy cutoff with an existing same-account binding. Account recreation can recover only a provider-verified token already attached to a non-retired durable principal whose personal account IDs are fully detached; an attached purchase cannot cross accounts. A newly recreated account's strictly free-only bootstrap principal may be retired after in-flight reservations are refunded and response data is scrubbed, preventing ordinary free use from blocking verified paid-principal recovery.
- Durable authority: a new opaque billing principal keeps hashed purchase lineage, terminal authority, wallet balances, allowance grants, and scrubbed AI settlement records independent of an Auth row. Auth deletion detaches personal identifiers and removes Planner replay/quote content; verified reattachment restores the same principal without issuing another grant.
- Client truth: canceled, pending, failed verification, and failed acknowledgement preserve previously verified access but never claim a new activation. Restore uses an explicit past-purchase query and then refreshes server authority. Receipt payloads are type checked before use.
- Local verification: fatal-info analysis, 93 focused billing/paywall/cleanup tests across the nine changed Phase 8 test files, the 1,930-test full Flutter suite with one expected QA-only skip, 15 QA-defined tests, 68 Edge Function tests, formatting, architecture, migration-policy, both secret guards, release, version, 11 workflow, and 16 Maestro checks passed.
- Blocked external evidence: the Phase 8 migration and 140-assertion pgTAP contract have not executed on PostgreSQL because no disposable local database is available. Database lint, deployed migration/grant/RLS/function parity, Play license-tester purchase/renewal/re-subscription/upgrade/downgrade/pending/cancel/grace/hold/recovery/hold-time-repurchase/pause/expire/revoke/refund/restore/account-recreation flows, RTDN delivery, process-death recovery, and signed-device evidence remain unverified. The recheck queue stores only token hashes, so an approved worker must reacquire a usable purchase token before it can query Google Play.
- Rollback: revert the app/Edge checkpoint only after containment is reverified. Once applied to a database, correct the Phase 8 schema only with an approved forward migration; never delete billing lineage or terminal authority to roll it back.

## Phase 9 Checkpoint - Privacy, Accessibility, Localization, And Measurement Boundaries

- Status: `BLOCKED_EXTERNAL`; launch verdict remains `NO-GO - NOT READY`.
- Privacy containment: Analytics and Crashlytics remain disabled by default and are additionally gated by account-scoped Settings consent. Sign-out resets runtime collection. Remote Crashlytics receives only a bounded diagnostic code and stack trace; it receives no raw account UID, planning content, notes, reflections, emotion data, prompts, breadcrumbs, arbitrary event text, or exception bodies. The canonical privacy policy matches this behavior.
- Accessibility: keyboard activation is available for the shared pressable control, press animation honors the platform reduced-motion preference, and the voice rationale is rendered before any microphone permission request. Local widget coverage includes the existing 200% first-setup path plus the new keyboard, reduced-motion, and rationale checks.
- Localization: the release advertises and supports English only. Existing Spanish translation work is preserved in source, but Spanish is removed from the supported-locale list so an unsupported device locale falls back to the verified English journey rather than presenting an incomplete mixed-language flow.
- Measurement: no dependency or asset declaration was removed without proven non-use. The checked-in raster/vector asset set is 55 files and 9.16 MiB, within the Phase 9 source budget of 10 MiB. Cold-start, jank, memory, battery, network, and signed-artifact size budgets still require physical signed-device measurement.
- Local verification: 22 focused accessibility, permission, telemetry, localization, and Settings tests; 38 focused legal, localization, startup, and release-contract tests; 13 focused telemetry/containment/startup tests; fatal-info analysis; generator verification; and formatting checks passed. No deployment, migration, credential change, paid purchase, external AI request, or cloud restore/sync occurred.
...
| BILL-01 | P1 | Purchase lineage and lifecycle ordering have authority edge cases | 8 | BLOCKED_EXTERNAL | source authority is hardened; paywall and paid credit plans remain disabled | Phase 8 checkpoint | Local client, Edge, static migration, and pgTAP source evidence pass; disposable PostgreSQL, deployed backend, Play sandbox, RTDN, and signed-device proof remain open |
...
| L10N-01 | P1 | English/Spanish product coverage is incomplete | 9 | PASS | release support is truthfully English-only; partial Spanish source is preserved and unsupported locales fall back to English | Phase 9 checkpoint | 38 focused legal/localization/startup/release-contract tests; full Spanish journey intentionally not advertised |
| PERF-01 | P2 | Bundle/dependency weight is unmeasured and likely wasteful | 9/10 | PASS (SOURCE) | 55 checked-in assets total 9.16 MiB, within the 10 MiB source budget; no unproven removal was made | Phase 9 checkpoint | source measurement only; signed-artifact and physical-device budgets remain Phase 10 external evidence |

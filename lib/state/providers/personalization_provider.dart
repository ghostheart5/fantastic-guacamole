...
const String personalizationProfileStorageKey =
...
final observedPlanningPatternsProvider = NotifierProvider<
...
class PersonalizationProfileController
  ...
  Future<void> updateGoalCategory(String value) => update(
    state.copyWith(goalCategory: value.trim()),
  );
...
class ObservedPlanningPatternsController
  ...
  Future<void> recordCompletion({required int difficulty}) => _persist(
    state.copyWith(
      ...
      shortTaskCompletions: state.shortTaskCompletions +
          (difficulty <= 2 ? 1 : 0),
      deepTaskCompletions: state.deepTaskCompletions +
          (difficulty >= 4 ? 1 : 0),
...
final personalizationDecisionProvider = Provider.family<
...
>((ref, surface) {
  ...
  return PersonalizationDecision(
    surface: surface,
    signalsUsed: signals,
    confidence: ((patterns.completed + patterns.skipped) / 20).clamp(0.0, 1.0),

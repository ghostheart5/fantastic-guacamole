...
class CompletionScoringEngine {
  CompletionScore calculate({
    ...
    if (quality >= 0.8) {
      feedback = 'Excellent completion — peak performance.';
    } else if (quality >= 0.5) {
      feedback = 'Solid effort — keep building momentum.';
    } else if (quality >= 0.25) {
      feedback = 'Good start — push a bit further next time.';
    } else {
      feedback = 'Short effort — every rep counts.';
    }

    return CompletionScore(
      xp: ProgressionPolicy.completionXp,
      quality: quality,
      feedback: feedback,

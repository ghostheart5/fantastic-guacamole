...
class UserGrowthEngine {
  ...
  UserGrowthState update(
    ...
    return UserGrowthState(
      ...
      adaptationRate: (current.adaptationRate + consistency * 0.01).clamp(0.0, 1.0),
      growthVelocity: (current.growthVelocity + streakBoost * 0.015).clamp(0.0, 1.0),
    );
  }

  String growthTitle(UserGrowthState state) {
    final double avg = (state.skillProgress + state.adaptationRate + state.growthVelocity) / 3.0;

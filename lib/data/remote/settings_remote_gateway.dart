...
class SettingsRemoteGateway {
  const SettingsRemoteGateway(this._client);
  ...
  Future<void> upsert({required Map<String, dynamic> row}) async {
    ...
    await client
        .from('settings')
        .upsert(_ownedRow(client, row), onConflict: 'user_id,id');
  }
  ...
  Map<String, dynamic> _ownedRow(
    ...
    if (userId == null || userId.isEmpty) {
      throw StateError('An authenticated user is required for settings sync.');
    }
    final String? claimedUserId = row['user_id']?.toString();
    if (claimedUserId != null &&
        claimedUserId.isNotEmpty &&
        claimedUserId != userId) {
      throw StateError(
        'Settings sync payload user does not match the active user.',
      );
    }

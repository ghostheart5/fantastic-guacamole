...
class LinkEmotionToGoalUsecase {
  const LinkEmotionToGoalUsecase(
  ...
  Future<TimelineEventEntity?> call({
    ...
    final TimelineEventEntity event = TimelineEventEntity(
      id: 'goal.link.emotion${targetGoalId}_${targetItemId}_${now.microsecondsSinceEpoch}',
      type: TimelineEventType.reflection,
      title: 'Emotion linked to goal',
      detail: 'Emotion linked $targetItemId for goal "${goal.title}".',
      timestamp: now,
      status: TimelineEventStatus.info,
      phase: 'goal.link.emotion',

...
class ArchiveGoalUsecase {
  const ArchiveGoalUsecase(this._goalRepository, this._timelineRepository);
  ...
  Future<TimelineEventEntity?> call(String goalId) async {
    ...
    final TimelineEventEntity event = TimelineEventEntity(
      id: 'goal_archive_${goal.id}_${now.microsecondsSinceEpoch}',
      type: TimelineEventType.reflection,
      title: 'Goal archived',
      detail: 'Archived goal "${goal.title}".',
      timestamp: now,
      status: TimelineEventStatus.info,
      phase: 'goal.metadata.archive',
      relatedId: goal.id,
    );

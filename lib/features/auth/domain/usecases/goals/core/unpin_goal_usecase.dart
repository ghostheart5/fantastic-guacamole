...
class UnpinGoalUsecase {
  const UnpinGoalUsecase(this._goalRepository, this._timelineRepository);
  ...
  Future<TimelineEventEntity?> call(String goalId) async {
    ...
    final TimelineEventEntity event = TimelineEventEntity(
      id: 'goal_unpin_${goal.id}_${now.microsecondsSinceEpoch}',
      type: TimelineEventType.reflection,
      title: 'Goal unpinned',
      detail: 'Unpinned goal "${goal.title}".',
      timestamp: now,
      status: TimelineEventStatus.info,
      phase: 'goal.metadata.unpin',
      relatedId: goal.id,
    );

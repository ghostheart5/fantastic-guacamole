...
class PinGoalUsecase {
  const PinGoalUsecase(this._goalRepository, this._timelineRepository);
  ...
  Future<TimelineEventEntity?> call(String goalId) async {
    ...
    final TimelineEventEntity event = TimelineEventEntity(
      id: 'goal_pin_${goal.id}_${now.microsecondsSinceEpoch}',
      type: TimelineEventType.reflection,
      title: 'Goal pinned',
      detail: 'Pinned goal "${goal.title}".',
      timestamp: now,
      status: TimelineEventStatus.info,
      phase: 'goal.metadata.pin',
      relatedId: goal.id,
    );

...
class CreateFitnessGoalUsecase {
  const CreateFitnessGoalUsecase(this._repository);
  ...
  Future<GoalEntity> call({
    ...
    final GoalEntity goal = GoalEntity(
      id: 'fitness_goal_${now.microsecondsSinceEpoch}',
      title: title.trim().isEmpty ? 'Fitness goal' : title.trim(),
      createdAt: now,
      description: description?.trim().isEmpty ?? true
          ? 'Fitness goal created in ChronoSpark.'
          : description?.trim(),

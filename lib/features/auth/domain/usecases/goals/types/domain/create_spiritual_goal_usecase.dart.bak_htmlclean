...
class CreateSpiritualGoalUsecase {
  const CreateSpiritualGoalUsecase(this._repository);
  ...
  Future<GoalEntity> call({
    ...
    final GoalEntity goal = GoalEntity(
      id: 'spiritual_goal_${now.microsecondsSinceEpoch}',
      title: title.trim().isEmpty ? 'Spiritual goal' : title.trim(),
      createdAt: now,
      description: description?.trim().isEmpty ?? true
          ? 'Spiritual goal created in ChronoSpark.'
          : description?.trim(),

  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    Future<void> completeEntry(Task entry) async {
      await ref
          ...
          .completeTask(entry.id, actionSource: 'creator_lists');
      if (!context.mounted) {
        return;
      }
    ...
    Future<void> delayEntry(Task entry) async {
      await ref
          .read(taskActionsProvider)
          .delayTask(entry.id, actionSource: 'creator_lists');
      if (!context.mounted) {
        return;
      }
      ScaffoldMessenger.of(context)
    ...
    Future<void> skipEntry(Task entry) async {
      await ref
          .read(taskActionsProvider)
          .skipTask(entry.id, actionSource: 'creator_lists');
      if (!context.mounted) {
        return;
      }
      ScaffoldMessenger.of(context)
...
class _CreatorEntryEditSheet extends StatefulWidget {
  const _CreatorEntryEditSheet({required this.entry, required this.onSave});

  final Task entry;
  final Future<void> Function(String title, String? description, int priority)
  onSave;
